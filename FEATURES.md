...
| F35 | Replicar o padrão de UX do F34 nas outras 19 calculadoras | `type: 'currency'`/`'stepper'`/`'itemList'` já existem prontos no `CalculatorForm` compartilhado (só faltou usar) — candidatas óbvias: `rescisao-trabalhista`, `ferias`, `decimo-terceiro`, `hora-extra` (todas com campos de moeda/dependentes/descontos parecidos). Priorizar pelas mesmas calculadoras do backlog de tráfego (F15/P0) primeiro. Paulo escolhe quando entrar na fila |
