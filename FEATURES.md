...
| F16 | `llms.txt` + GEO (SEO de IA) — **promovido de P2 para P0** | Canal "AI Assistant" (ChatGPT/Copilot) já traz ~17 sessões/mês **de graça, sem nenhuma otimização feita ainda** — sinal validado de que geração de resposta por IA já cita o site; formalizar com `llms.txt` (Recibo Fácil já tem, `frontend/public/llms.txt`) é o próximo passo óbvio e barato |
